        ...
        .factory('formPostData', [
            ...
            'tubularTemplateService', 'formPostData', function tubularGenerator(tubularTemplateService, formPostData) {
                ...
                me.runFormTemplate = function(scope) {
                    var layout = scope.formOptions.Layout == 'Simple' ? '' : scope.formOptions.Layout.toLowerCase();

                    return '<tb-form layout="' + layout + '" server-save-method="' + scope.formOptions.SaveMethod + '" ' +
                        'model-key="' + scope.formOptions.ModelKey + '"' +
                        'server-url="' + scope.dataUrl + '" server-save-url="' + scope.formOptions.SaveUrl + '" ' +
                        (scope.isOData ? ' service-name="odata"' : '') + '>' +
                        '\r\n\t<h1>Autogenerated Form</h1>' +
                        tubularTemplateService.generateFields(scope.columns) +
                        '\r\n\t<tb-form-buttons show-cancel="' + scope.formOptions.CancelButton + '"></tb-form-buttons>' +
                        '\r\n</tb-form>';
                };
