(function(){"use strict";angular.module("tubular-hchart.directives",["tubular.services","highcharts-ng"]).component("tbHighcharts",{template:'<div class="tubular-chart"><highchart config="$ctrl.options" ng-hide="$ctrl.isEmpty || $ctrl.hasError"><\/highchart><div class="alert alert-info" ng-show="$ctrl.isEmpty">{{$ctrl.emptyMessage}}<\/div><div class="alert alert-warning" ng-show="$ctrl.hasError">{{$ctrl.errorMessage}}<\/div><\/div>',bindings:{serverUrl:"@",title:"@?",requireAuthentication:"=?",name:"@?chartName",chartType:"@?",emptyMessage:"@?",errorMessage:"@?",onLoad:"=?",options:"=?",onClick:"=?"},controller:["$scope","tubularHttp","$timeout",function(n,t,i){var r=this;r.dataService=t.getDataService(r.dataServiceName);r.showLegend=angular.isUndefined(r.showLegend)?!0:r.showLegend;r.chartType=r.chartType||"line";r.options=angular.extend({},r.options,{options:{chart:{type:r.chartType},plotOptions:{pie:{point:{events:{click:r.onClick||angular.noop}}},series:{point:{events:{click:r.onClick||angular.noop}}}}},title:{text:r.title||""},xAxis:{categories:[]},yAxis:{},series:[]});r.requireAuthentication=angular.isUndefined(r.requireAuthentication)?!0:r.requireAuthentication;r.loadData=function(){t.setRequireAuthentication(r.requireAuthentication);r.hasError=!1;t.get(r.serverUrl).promise.then(function(n){if(!n||!n.Data||n.Data.length===0){if(r.isEmpty=!0,r.options.series=[{data:[]}],r.onLoad)r.onLoad(r.options,{});return}if(r.isEmpty=!1,n.Series)r.options.xAxis.categories=n.Labels,r.options.series=n.Series.map(function(t,i){return{name:t,data:n.Data[i]}});else{var t=n.Labels.map(function(t,i){return{name:t,y:n.Data[i]}});r.options.series=[{name:n.SerieName||"",data:t,showInLegend:(n.SerieName||"")!=""}]}if(r.onLoad){i(function(){r.onLoad(r.options,{},r.options.getHighcharts().series)},100);r.onLoad(r.options,{},null)}},function(t){n.$emit("tbChart_OnConnectionError",t);r.hasError=!0})};n.$watch("$ctrl.serverUrl",function(n){angular.isDefined(n)&&n!=null&&n!=""&&r.loadData()});n.$watch("$ctrl.chartType",function(n){angular.isDefined(n)&&n!=null&&(r.options.options.chart.type=n)})}]})})();
/**
 * highcharts-ng
